...
static const Pin_Map pinMap[NUM_DIGITAL_PINS] = {
    ...
    [2] = { .ddr=&DDRD, .pin=&PIND, .port=&PORTD, .bit= PD2 }, // {18} (D 2) INT0 [FTDI_!DTR!]
    [3] = { .ddr=&DDRD, .pin=&PIND, .port=&PORTD, .bit= PD3 }, // {19} (D 3) INT1 OC2B [FTDI_!RTS!]
    [4] = { .ddr=&DDRD, .pin=&PIND, .port=&PORTD, .bit= PD4 }, // {20} (D 4) T0 [RX_!RE!]
    [5] = { .ddr=&DDRD, .pin=&PIND, .port=&PORTD, .bit= PD5 }, // {21} (D 5) T1 OC0B [TX_DE]
