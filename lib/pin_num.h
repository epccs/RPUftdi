...
static const Pin_Map pinMap[NUM_DIGITAL_PINS] = {
    ...
    [2] = { .ddr=&DDRD, .pin=&PIND, .port=&PORTD, .bit= PD2 }, // {18} INT0 [FTDI_!DTR!]
    [3] = { .ddr=&DDRD, .pin=&PIND, .port=&PORTD, .bit= PD3 }, // {19} INT1 OC2B [FTDI_!RTS!]
    ...
    [14] = { .ddr=&DDRC, .pin=&PINC, .port=&PORTC, .bit= PC0 }, // {8} ADC0 [FTDI_!CTS!]
    [15] = { .ddr=&DDRC, .pin=&PINC, .port=&PORTC, .bit= PC1 }, // {9} ADC1 [FTDI_!DSR!]
